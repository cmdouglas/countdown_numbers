...
class BinaryIntegerArithmeticExpression:
    ...
    def __init__(self, value=None, op1=None, op2=None, o=None):
        if value is not None:
            ...
            if o == '/' and (op1.value % op2.value != 0):
                raise InvalidExpressionException("Division is only allowable "
                                                 "if the quotient is an" 
                                                 "integer.")
            self.op1 = op1
            self.op2 = op2
            self.o = o
    ...
    def rep(self, parent_op=None, side=None):
        if self.o is None:
            ...
            # we also need parens if we're on the right side of a left-associative operation
            # and our parent is not associative
            # i.e 4 - 2 + 2 vs 4 - (2 + 2)
    ...
    def parse(cls, s):
        ...
        for item in output_queue:
            if type(item) == int:
                ...
                operand2 = stack.pop()
                operand1 = stack.pop()
                stack.append(BinaryIntegerArithmeticExpression(op1=operand1, op2=operand2, o=item))
...
if __name__ == '__main__':
    ...
    for test in tests:
        ...
        print ("input {test} resulted in expression {e} with value {v}".format(
            test=test,
            e=str(e),
            v=e.value
        ))
